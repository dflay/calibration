...
int GetOmegaP_free(perturbation_t pert,double freq,double freqErr,double temp,double tempErr,
   ...
   double delta_t  = GetDeltaTerm(sigma,pert.delta_m,chi,pert.delta_eps,pert.delta_mag);
   // error on delta_t  
   double err_sq   = pert.sigma_err*pert.sigma_err + pert.delta_m_err*pert.delta_m_err 
                   + pert.delta_eps_err*pert.delta_eps_err + pert.delta_mag_err*pert.delta_mag_err;
...
int GetDeltaTerm(double sigma,double delta_m,double chi,double delta_eps,double delta_mag){
   ...
   double delta_t = 1E-9*(sigma - delta_m + (eps-4.*TMath::Pi()/3.) - delta_eps - delta_mag);
   return delta_t;
}
//______________________________________________________________________________
